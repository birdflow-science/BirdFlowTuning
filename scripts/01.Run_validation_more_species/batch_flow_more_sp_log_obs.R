...
the$python_repo_path <- '/work/pi_drsheldon_umass_edu/birdflow_modeling/pipeline/BirdFlowPy_modified'
assign("the", the, envir = .GlobalEnv)
...
#### Batch fit
unique_names <- c('amewoo', 'buwtea', 'swahaw', 'brwhaw', 'woothr', 'yelwar') #unique_names #c('ovenbi1', 'paibun', 'whimbr', 'lobcur', 'osprey', 'tunswa', 'bkbplo') #c('lobcur') #c('amewoo', 'buwtea', 'swahaw', 'brwhaw', 'woothr') #, 'lobcur',
unique_names <- c('treswa')
for (sp in unique_names){
  print(sp)
  sp_output_path <- paste0('/project/pi_drsheldon_umass_edu/birdflow/batch_model_validation/model_output_hyperparams_distance_metric_log_obs','/',sp)
  if (!dir.exists(sp_output_path)){dir.create(sp_output_path, recursive = TRUE)}

  ## Load
  file1 <- glue::glue("/project/pi_drsheldon_umass_edu/birdflow/batch_model_validation/model_output_hyperparams_distance_metric_log_obs/{sp}/{sp}_150km_interval_based_eval_using_migration_transitions/eval_metrics_train_distance_metric_all_combined.rds")
  file2 <- glue::glue("/project/pi_drsheldon_umass_edu/birdflow/batch_model_validation/model_output_hyperparams_distance_metric_log_obs/{sp}/{sp}_150km_interval_based_eval_using_migration_transitions/eval_metrics_test_distance_metric_all_combined.rds")
